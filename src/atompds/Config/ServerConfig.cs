...
public record ServerConfig
{
    ...
    public SubscriptionConfig MapSubscriptionConfig(ServerEnvironment env)
    {
    ...
    {
        return !string.IsNullOrEmpty(env.PDS_DATA_DIRECTORY)
            ? Path.Combine(env.PDS_DATA_DIRECTORY, name)
            : name;
    }
    public DatabaseConfig MapDatabaseConfig(ServerEnvironment env)
    {
    ...
    public ActorStoreConfig MapActorStoreConfig(ServerEnvironment env)
    {
        return new ActorStoreConfig
        {
    ...
    public IdentityConfig MapIdentityConfig(ServerEnvironment env)
    {
    ...
    public InvitesConfig MapInviteConfig(ServerEnvironment env)
    {
        return env.PDS_INVITE_REQUIRED
    ...
    {
        return env.PDS_BSKY_APP_VIEW_URL != null
    ...
    public SecretsConfig MapSecretsConfig(ServerEnvironment env)
    {
        return new SecretsConfig
        {
            JwtSecret = env.PDS_JWT_SECRET,
            PlcRotationKey = Secp256k1Keypair.Import(env.PDS_PLC_ROTATION_KEY_K256_PRIVATE_KEY_HEX)
        };
    }

    public ProxyConfig MapProxyConfig(ServerEnvironment env)
    {
    ...
    public DiskBlobstoreConfig MapDiskBlobstoreConfig(ServerEnvironment env)
    {
        return new DiskBlobstoreConfig
        {
