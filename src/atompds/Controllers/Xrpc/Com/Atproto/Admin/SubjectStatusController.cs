...
public class SubjectStatusController : ControllerBase
{
    ...
    private readonly AccountManagerDb _accountManagerDb;
    ...
    public SubjectStatusController(
        AccountRepository accountRepository,
        ActorRepositoryProvider actorRepositoryProvider,
        AccountManagerDb accountManagerDb,
        ILogger<SubjectStatusController> logger)
    {
        _accountRepository = accountRepository;
        _actorRepositoryProvider = actorRepositoryProvider;
        _accountManagerDb = accountManagerDb;
        _logger = logger;
    }

    [HttpGet("com.atproto.admin.getSubjectStatus")]
    [AdminToken]
    public async Task<IActionResult> GetSubjectStatusAsync([FromQuery] string? did, [FromQuery] string? uri, [FromQuery] string? blob)
    {
        EnsureSingleSubjectSelector(did, uri, blob);

        if (!string.IsNullOrWhiteSpace(did))
        {
            var account = await GetRequiredAccountAsync(did);
            return Ok(new GetSubjectStatusOutput
            {
        ...
        {
            var recordStatus = await GetRecordStatusAsync(ParseAtUri(uri));
            return Ok(new GetSubjectStatusOutput
            {
                Subject = new StrongRefSubject
                {
                    Uri = recordStatus.Uri,
                    Cid = recordStatus.Cid
                },
                Takedown = ToStatusAttr(recordStatus.TakedownRef)
            });
        }

        var blobStatus = await GetBlobStatusAsync(blob!);
        return Ok(new GetSubjectStatusOutput
        {
            Subject = new RepoBlobRefSubject
            {
                Did = blobStatus.Did,
                Cid = blobStatus.Cid,
                RecordUri = blobStatus.RecordUri
            },
    ...
    public async Task<IActionResult> UpdateSubjectStatusAsync([FromBody] UpdateSubjectStatusInput request)
    {
        ...
        switch (subject)
        {
            ...
            {
                await GetRequiredAccountAsync(repoSubject.Did);

                if (request.Takedown != null)
                {
                    await _accountRepository.UpdateTakedownRefAsync(repoSubject.Did, request.Takedown.Applied ? request.Takedown.Ref ?? DefaultTakedownRef : null);
                }
                ...
                if (request.Deactivated != null)
                {
                    ...
                    {
                        await _accountRepository.DeactivateAccountAsync(repoSubject.Did, null);
                    }
                    else
                    {
                        await _accountRepository.ActivateAccountAsync(repoSubject.Did);
                    }
                }

                var account = await GetRequiredAccountAsync(repoSubject.Did);
                return Ok(new UpdateSubjectStatusOutput
                {
                    Subject = new RepoRefSubject { Did = account.Did },
                    Takedown = ToStatusAttr(account.TakedownRef)
                });
            ...
            case RecordSubjectInput recordSubject:
            {
                ...
                var updatedRecord = await UpdateRecordStatusAsync(recordSubject, request.Takedown);
                return Ok(new UpdateSubjectStatusOutput
                {
                    Subject = new StrongRefSubject
                    {
            ...
            case BlobSubjectInput blobSubject:
            {
                ...
                var updatedBlob = await UpdateBlobStatusAsync(blobSubject, request.Takedown);
                return Ok(new UpdateSubjectStatusOutput
                {
                    Subject = new RepoBlobRefSubject
                    {
                        Did = updatedBlob.Did,
                        Cid = updatedBlob.Cid,
                        RecordUri = updatedBlob.RecordUri
                    },
    ...
    {
        return new StatusAttr
        {
            Applied = refValue != null,
            Ref = refValue
        };
    }
    ...
    private async Task<ActorAccount> GetRequiredAccountAsync(string did)
    {
        var account = await _accountRepository.GetAccountAsync(did, new AvailabilityFlags(true, true));
        if (account == null)
        {
            throw InvalidRequest("Account not found");
        }

        return account;
    }

    private async Task<RecordSubjectState> GetRecordStatusAsync(ATUri subjectUri)
    {
        var did = subjectUri.Authority;
        if (string.IsNullOrWhiteSpace(did))
        {
            throw InvalidRequest("uri must include a DID authority");
        }

        await GetRequiredAccountAsync(did);
        ...
        if (record == null)
        {
            throw InvalidRequest("Record not found");
        }

        return new RecordSubjectState(record.Uri, record.Cid, record.TakedownRef);
    }

    private async Task<RecordSubjectState> UpdateRecordStatusAsync(RecordSubjectInput subject, StatusAttr? takedown)
    {
    ...
    private async Task<BlobSubjectState> GetBlobStatusAsync(string cid)
    {
        BlobSubjectState? match = null;

        var dids = await _accountManagerDb.Actors
            .AsNoTracking()
            .Select(x => x.Did)
            .ToListAsync();
        ...
        foreach (var did in dids)
        {
            ...
            var candidate = await actorRepo.TransactDbAsync(async db =>
            {
                ...
                var recordUri = await db.RecordBlobs
                    .AsNoTracking()
            ...
            if (match != null)
            {
                _logger.LogWarning("Blob {Cid} matched multiple repos while resolving subject status", cid);
    ...
    private async Task<BlobSubjectState> UpdateBlobStatusAsync(BlobSubjectInput subject, StatusAttr? takedown)
    {
        await GetRequiredAccountAsync(subject.Did);
        await using var actorRepo = _actorRepositoryProvider.Open(subject.Did);
        ...
        var updated = await actorRepo.TransactDbAsync(async db =>
        {
            ...
            {
                var hasRecordUri = await db.RecordBlobs
                    .AsNoTracking()
                    .AnyAsync(x => x.BlobCid == subject.Cid && x.RecordUri == subject.RecordUri);

                if (!hasRecordUri)
                {
                    throw InvalidRequest("Blob not found for recordUri");
                }
            }
            ...
            var recordUri = subject.RecordUri ?? await db.RecordBlobs
                .AsNoTracking()
    ...
    private static void EnsureSingleSubjectSelector(string? did, string? uri, string? blob)
    {
        ...
        if (supplied != 1)
        {
            throw InvalidRequest("exactly one of did, uri, or blob is required");
        }
    }
    ...
    private static SubjectInput NormalizeSubject(UpdateSubjectStatusInput request)
    {
        ...
        if (hasDid && !hasUri && !hasCid && !hasRecordUri)
        {
            return new RepoSubjectInput(request.Subject.Did!);
        }

        if (!hasDid && hasUri && hasCid && !hasRecordUri)
        {
            return new RecordSubjectInput(ParseAtUri(request.Subject.Uri!), request.Subject.Cid!);
        }

        if (hasDid && !hasUri && hasCid)
        {
            return new BlobSubjectInput(request.Subject.Did!, request.Subject.Cid!, request.Subject.RecordUri);
        }
    ...
    private static ATUri ParseAtUri(string uri)
    {
        ...
        {
            throw InvalidRequest("invalid at-uri");
        }
    }

    private static XRPCError InvalidRequest(string message)
        => new(new InvalidRequestErrorDetail(message));
