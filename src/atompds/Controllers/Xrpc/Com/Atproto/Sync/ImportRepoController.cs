...
[ApiController]
[Route("xrpc")]
public class ImportRepoController : ControllerBase
{
    private readonly ActorRepositoryProvider _actorRepositoryProvider;
    private readonly ILogger<ImportRepoController> _logger;

    public ImportRepoController(
        ActorRepositoryProvider actorRepositoryProvider,
        ILogger<ImportRepoController> logger)
    {
    ...
    public async Task<IActionResult> ImportRepoAsync()
    {
        ...
        _logger.LogInformation("Importing repo {Did} with root {RootCid}, {BlockCount} blocks", did, rootCid, blocks.Count);
        ...
        await actorStore.TransactRepoAsync(async repo =>
        {
            foreach (var block in blocks)
            {
                ...
                {
                    return await db.RepoBlocks.FindAsync(block.Cid.ToString()) != null;
                });
                ...
                if (existing != true)
                {
                    ...
                    {
                        db.RepoBlocks.Add(new ActorStore.Db.RepoBlock
                        {
    ...
    private (CarHeader header, List<CarBlock> blocks) ParseCarFile(byte[] carBytes)
    {
        var offset = 0;
