...
public class RepoTransactor
{
    ...
    private readonly RecordTransactor _record;
    private readonly DateTime _now = DateTime.UtcNow;
    public RepoTransactor(ActorStoreDb db, string did, IKeyPair keyPair, SqlRepoTransactor storage, RecordTransactor record)
    {
        _db = db;
        _did = did;
        _keyPair = keyPair;
    ...
    public async Task<CommitData> CreateRepo(PreparedCreate[] writes)
    {
        ...
        var commit = await Repo.Repo.FormatInitCommit(_storage, _did, _keyPair, writeOpts);
...
public class RecordTransactor
{
    private readonly ActorStoreDb _db;
    private readonly string _did;
    private readonly IKeyPair _keyPair;
    private readonly SqlRepoTransactor _storage;
    public RecordTransactor(ActorStoreDb db, string did, IKeyPair keyPair, SqlRepoTransactor storage)
    {
        _db = db;
        _did = did;
        _keyPair = keyPair;
